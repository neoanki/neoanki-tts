...
test('installs the full extension and keeps provider credentials behind the secret broker', async () => {
  ...
  const packageName = (await readdir(join(extensionRoot, 'build'))).find((name) => name === 'org.neoanki.tts-2.1.0.neoanki-extension')
  ...
  try {
    ...
    await expect(window.getByText(/Cloud tracks send processed text/i)).toBeVisible()
    await expect(window.locator('iframe[title*="settings" i]')).toHaveCount(0)
    await expect(window.getByLabel('OpenAI API key', { exact: true })).toBeVisible()
    await expect(window.getByText('Not configured').first()).toBeVisible()
    ...
    const apiKey = window.getByLabel('OpenAI API key', { exact: true })
    ...
    await window.getByRole('combobox', { name: /^Provider(?:\s+\*)?$/ }).first().selectOption('openai')
    await window.getByRole('combobox', { name: /^Playback mode(?:\s+\*)?$/ }).first().selectOption('generated')
    await window.getByRole('textbox', { name: 'Voice ID or name', exact: true }).first().fill('coral')
    await window.getByRole('button', { name: 'Save settings' }).click()
    await expect(window.getByText('Settings saved to the encrypted workspace.')).toBeVisible({ timeout: 30_000 })
    ...
    await window.getByRole('button', { name: /^Extensions/ }).first().click()
    await window.getByRole('tab', { name: /configure/i }).click()
    const speed = window.getByRole('slider', { name: /^Speech speed(?:\s+\*)?$/ }).first()
    await speed.fill('1.1')
    const updatedSave = window.getByRole('button', { name: 'Save settings' })
    await expect(updatedSave).toBeEnabled()
    await updatedSave.click()
    await expect(window.getByText('Settings saved to the encrypted workspace.')).toBeVisible({ timeout: 30_000 })
    ...
    await window.getByRole('button', { name: /^Extensions/ }).first().click()
    await window.getByRole('tab', { name: /configure/i }).click()
    await window.getByRole('slider', { name: /^Speech speed(?:\s+\*)?$/ }).first().fill('1.2')
    await expect(updatedSave).toBeEnabled()
    await updatedSave.click()
    await expect(window.getByText('Settings saved to the encrypted workspace.')).toBeVisible({ timeout: 30_000 })
