...
test('installs the full extension and keeps provider credentials behind the secret broker', async () => {
  ...
  try {
    ...
    await window.getByLabel('Provider', { exact: true }).first().selectOption('openai')
    await window.getByLabel('Playback mode', { exact: true }).first().selectOption('generated')
    await window.getByLabel('Voice ID or name', { exact: true }).first().fill('coral')
    ...
    await window.getByRole('button', { name: /^Extensions/ }).first().click()
    await window.getByRole('tab', { name: /configure/i }).click()
    const speed = window.getByLabel('Speech speed', { exact: true }).first()
    ...
    await window.getByRole('button', { name: /^Extensions/ }).first().click()
    await window.getByRole('tab', { name: /configure/i }).click()
    await window.getByLabel('Speech speed', { exact: true }).first().fill('1.2')
    await expect(updatedSave).toBeEnabled()
    await updatedSave.click()
    await expect(window.getByText('Settings saved to the encrypted workspace.')).toBeVisible({ timeout: 30_000 })
